...
# --- 2. LOAD SECRETS (Strict Mode) ---
# We prioritize Streamlit Secrets (Cloud). 
# If running locally, you MUST have .streamlit/secrets.toml
try:
    DATABASE_URL = st.secrets["DATABASE_URL"]
    API_KEY = st.secrets.get("UPSTOX_API_KEY", "")
    API_SECRET = st.secrets.get("UPSTOX_API_SECRET", "")
    # 🚨 SOURCE OF TRUTH: No guessing. Reads directly from Config.
    # Make sure REDIRECT_URI is set in your secrets.toml or Cloud Settings.
    REDIRECT_URI = st.secrets.get("REDIRECT_URI", "https://gemini-trading-bot-yash.streamlit.app")
except FileNotFoundError:
    # Fallback for local dev without secrets file (Env vars)
    DATABASE_URL = os.getenv("DATABASE_URL")
    API_KEY = os.getenv("UPSTOX_API_KEY")
    API_SECRET = os.getenv("UPSTOX_API_SECRET")
    REDIRECT_URI = os.getenv("REDIRECT_URI", "https://gemini-trading-bot-yash.streamlit.app")
...
with st.sidebar:
    ...
    # Debug: Show user where we are redirecting (Good for verification)
    st.caption(f"Redirect URI: `{REDIRECT_URI}`")
    
    if API_KEY and API_SECRET:
        # Use the REDIRECT_URI strictly
    ...
    st.caption("Last Update: " + time.strftime("%H:%M:%S"))
...
elif not upstox_client.fetch_token_from_db():
    st.warning("⚠️ Upstox Token Expired. Please Login.")
